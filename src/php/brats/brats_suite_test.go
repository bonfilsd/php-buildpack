...
func FindModules(version string) []string {
	m := struct {
		Dependencies struct {
	...
	bpDir, err := cutlass.FindRoot()
	Expect(err).ToNot(HaveOccurred())
	Expect(libbuildpack.NewYAML().Load(filepath.Join(bpDir, "manifest.yml"), &m)).To(Succeed())

	Fail("Could not find modules for php " + version)
	return []string{"PretendModule"}
}
